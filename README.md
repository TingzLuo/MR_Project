...
一个基于 SCG（结构化内容图）的 MR（蜕变关系，Metamorphic Relations）推荐系统。系统通过解析文档生成 SCG 结构化内容图，再基于 SCG 智能推荐蜕变关系，辅助软件测试中的蜕变测试工作。
...
```
文档上传 → 文档解析 → SCG 生成 → MR 推荐 → 蜕变关系结果
```

1. **文档上传与解析**：用户上传 PDF/Word 文档，系统自动解析文档内容
2. **SCG 生成**：基于文档内容，通过 LLM 生成结构化内容图（Structure Content Graph），以节点和边的形式表达文档的语义结构
3. **MR 推荐**：基于生成的 SCG，系统智能推荐相关的蜕变关系（Metamorphic Relations）
4. **结果管理**：用户可对推荐的蜕变关系进行查看、编辑和确认保存
...
| MR 推荐 | 基于 SCG 智能推荐蜕变关系，支持结果查看与保存 |
| 操作记录 | 查看历史操作活动 |
| 个人中心 | 修改个人信息、修改密码 |
